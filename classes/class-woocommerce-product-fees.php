...
class WooCommerce_Product_Fees {
	 ...
	 */
	private function product_contains_fee_data( $id ) {
	 ...
	 */
	private function make_percentage_adjustments( $fee_amount, $item_price ) {
	 ...
	 */
	private function maybe_multiply_by_quantity( $amount, $multiplier, $qty ) {
	 ...
	 */
	private function get_fee_data( $item ) {
		$fee_data = false;
	 ...
	 */
	private function maybe_remove_fees_for_coupon( $cart ) {
	 ...
	 */
	private function get_fee_tax_class( $product ) {
	 ...
	 */
	private function get_fees( $cart ) {
		$fees = array();
			...
			if ( $fee ) {
				...
				if ( array_key_exists( $fee_id, $fees ) && 'combine' === get_option( 'wcpf_name_conflicts', 'combine' ) ) {
					...
					$fees[$fee_id] = apply_filters( 'wcpf_filter_fee_data', array(
						'name' => $fee['name'],
						'amount' => $fee['amount'],
						'taxable' => ( wc_tax_enabled() && '_no_tax' !== $fee_tax_class ) ? true : false,
